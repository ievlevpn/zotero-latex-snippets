...
export function run() {
	...
	/* --- tabstops: first one selected, Tab walks the rest --- */
	{
		...
		assert.strictEqual(ls.setSelectionToNextTabstop(view, false), true);
		...
		assert.strictEqual(ls.setSelectionToNextTabstop(view, false), false, "no tabstops left");
		ls.clearTabstops();
	}
	...
	/* --- a text block: offsets and positions differ around an inline equation --- */
	{
		...
		const buffer = ls.Buffer.forTextBlock(view);
	...
	/* --- \n in a paragraph becomes a hard break, not a stray character --- */
	{
		...
		ls.Buffer.forTextBlock(view).applyChange(1, 1, "x\ny");
